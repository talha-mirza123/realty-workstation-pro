<?php
    ...
    $agent = get_user_by( 'ID', (int) get_post_meta( $transaction->ID, 'agent', true ) );
?>
<div class="realty-workstation-pro-agents-all edit-transaction-container">
    <h3><?php echo ucfirst($_GET['status']) ?> Transaction - <?php echo get_post_meta( $transaction->ID, 'type', true ); ?> <?php echo ($_SESSION['rwUser'] == 'broker') ? '- ' . $agent->first_name . ' ' . $agent->last_name : ''; ?></h3>
    <hr>
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="<?php echo admin_url('admin-post.php'); ?>">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            <?php
                                ...
                                $commission = get_user_meta( (int) $agent, 'commission', true );
                                $lease = get_user_meta( (int) $agent, 'lease', true );
                                $sale_and_purchase = get_user_meta( (int) $agent, 'sale_and_purchase', true );
                                $calculatedCommissionPercentage = $commission * 100;
                                if (isset($broker_referral) && !empty($broker_referral) && $broker_referral == 'true') {
                                    if ( $transactionType == 'Sale' || $transactionType == 'Purchase' ) {
                                        $calculatedCommissionPercentage = $sale_and_purchase * 100;
                                    }
                                    if ( $transactionType == 'Lease - Tenant' || $transactionType == 'Lease - Landlord' ) {
                                        $calculatedCommissionPercentage = $lease * 100;
                                    }
                                }
                            ?>
                            ...
                            <input type="text" id="agent_payout" class="form-control agent-payout mask-money" name="agent_payout" value="<?php echo get_post_meta( $transaction->ID, 'agent_payout', true ); ?>" readonly data-commission="<?php echo $commission; ?>" data-lease="<?php echo $lease; ?>" data-sale-and-purchase="<?php echo $sale_and_purchase; ?>" data-transaction-type="<?php echo $transactionType; ?>">
                        </div>
                ...
                <div class="button-group">
                    <button type="button" class="btn btn-success w-25 update-agent-transaction-btn">Save</button>
                    <button type="button" class="btn btn-primary w-25 close-agent-transaction-btn">Close and Request Payment</button>
                </div>
            </form>
        </div>
    </div>
